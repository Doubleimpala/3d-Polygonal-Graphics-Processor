...
void SPI_init() {
	...
	// ConfigPtr = XSpi_LookupConfig(XPAR_SPI_USB_DEVICE_ID);
	ConfigPtr = XSpi_LookupConfig(XPAR_SPI_USB_BASEADDR);
	if (ConfigPtr == NULL) {
		return XST_DEVICE_NOT_FOUND;
	}
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	if (XSpi_Transfer(&SpiInstance, buf, NULL, 2) != 0)
		xil_printf("Error: Failed single byte write");
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if (XSpi_Transfer(&SpiInstance, buf, NULL, nbytes+1) != 0)
		xil_printf("Error: Failed multibyte write");
...
BYTE MAXreg_rd(BYTE reg) {
	...
	write_buf[1] = 0x00;
 	if (XSpi_Transfer(&SpiInstance, write_buf, read_buf, 2) != 0)
		xil_printf("Error: Failed single byte read");
	
	xil_printf("Keycode: %u", read_buf[1]);
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return read_buf[1];
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	write_buf[0] = reg;
	for (int i = 1; i < nbytes + 1; i++) write_buf[i] = 0x00;
 	if (XSpi_Transfer(&SpiInstance, write_buf, read_buf, nbytes + 1) != 0)
		xil_printf("Error: Failed to read reg");
...
void MAX3421E_reset(void) {
	// Status = XGpio_Initialize(&Gpio_rst, XPAR_GPIO_USB_RST_DEVICE_ID);
	Status = XGpio_Initialize(&Gpio_rst, XPAR_GPIO_USB_RST_BASEADDR);
	XGpio_SetDataDirection(&Gpio_rst, 1, 0); //configure reset, and set reset to output
	// Status = XGpio_Initialize(&Gpio_int, XPAR_GPIO_USB_INT_DEVICE_ID);
	Status = XGpio_Initialize(&Gpio_int, XPAR_GPIO_USB_INT_BASEADDR);
	XGpio_SetDataDirection(&Gpio_int, 1, ~1); //configure int, and set int to input
...
void MAX3421E_init(void) {
	...
	//start USB timer
	// Status = XTmrCtr_Initialize(&Usb_timer, XPAR_TIMER_USB_AXI_DEVICE_ID);
	Status = XTmrCtr_Initialize(&Usb_timer, XPAR_TIMER_USB_AXI_BASEADDR);
	if (Status != XST_SUCCESS) {
			xil_printf ("Timer instantiation failed\n");
	}
	XTmrCtr_Start(&Usb_timer, 0);

...
Early plans for rasterization pipeline:  
![][image4]  
![][image5]  
State diagram:  
![][image6]  
...
Despite having faced many difficulties, we were able to get partial functionality. Everything seemed to work except our z-buffering, which failed on hardware but worked on simulation. It is possible we were not thorough enough in the simulation. Given that it seemed full functionality was right around the corner, we were very disappointed with our final demo. After at least 110 hours of work combined between both of us, we thought we would have more to show. We did work a little more that night and reduced the glitching by increasing precision, but we had to turn in our boards. Thus, we can no longer test on hardware either. However, we still wish to work on our own time to finish and add more features on our own time.
